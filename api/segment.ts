...
function setCors(res: any) {
  // If you want to restrict, set this to your framer domain(s).
...
async function apolloOrgEnrich(domain: string): Promise<{ employees?: number; apolloLatencyMs: number } | null> {
...
export default async function handler(req: any, res: any) {
  ...
  if (!domain) {
    res.status(400).json({ tier: "unknown", reason: "bad_email", totalLatencyMs: Date.now() - requestStart });
    return;
  }
